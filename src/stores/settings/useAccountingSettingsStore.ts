...
export const useAccountingSettingsStore = defineStore('accountingSettings', () => {
  const data = ref<AccountingSettings | null>(null);
  ...
  async function fetch(force = false) {
    ...
    error.value = null;
    try {
      const result = await settingsApi.accounting.get();
      if (result.ok) {
  ...
  async function update(payload: Partial<AccountingSettings>) {
    loading.value = true;
    error.value = null;
    try {
      const result = await settingsApi.accounting.update(payload);
      if (result.ok) {
        data.value = result.data;
      } else {
        error.value = result.error?.message ?? 'Unknown error';
