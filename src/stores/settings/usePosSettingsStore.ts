...
export const usePosSettingsStore = defineStore('posSettings', () => {
  const data = ref<PosSettings | null>(null);
  ...
  async function fetch(force = false) {
    ...
    error.value = null;
    try {
      const result = await settingsApi.pos.get();
      if (result.ok) {
  ...
  async function update(payload: Partial<PosSettings>) {
    loading.value = true;
    error.value = null;
    try {
      const result = await settingsApi.pos.update(payload);
      if (result.ok) {
        data.value = result.data;
      } else {
        error.value = result.error?.message ?? 'Unknown error';
