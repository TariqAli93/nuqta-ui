...
export const useBarcodeSettingsStore = defineStore('barcodeSettings', () => {
  const data = ref<BarcodeSettings | null>(null);
  ...
  async function fetch(force = false) {
    ...
    error.value = null;
    try {
      const result = await settingsApi.barcode.get();
      if (result.ok) {
  ...
  async function update(payload: Partial<BarcodeSettings>) {
    loading.value = true;
    error.value = null;
    try {
      const result = await settingsApi.barcode.update(payload);
      if (result.ok) {
        data.value = result.data;
      } else {
        error.value = result.error?.message ?? 'Unknown error';
